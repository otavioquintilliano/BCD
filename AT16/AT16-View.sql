...
SELECT MAX(salario), DataNasci, CPF
FROM funcionarios
WHERE DataNasci <= 1985 AND CPF = 'IS NULL';
...
FROM funcionarios
WHERE PrimeiroNome = 'Almeida' OR salario <= 3500; 
-- 13. Selecione o salário-mínimo entre os funcionários que trabalham como Designer E têm CPF cadastrado.

SELECT MIN(salario), funcao, CPF
FROM funcionarios
WHERE funcao = 'Designer' AND CPF = 'IS NULL';
